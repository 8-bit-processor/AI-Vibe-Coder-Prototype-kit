...
from attention_context import context_data
from attention_context.dependency_manager import DependencyManager
...
class ContextEngine:
    ...
    def __init__(self, project_root: Path, logger_func, context_data=None):
        """
        ...
        Args:
            ...
            context_data: Shared state container.
        """
    ...
    def project_goal(self):
        return self.context_data.project_goal if self.context_data else ""
    ...
    def update_module_map(self):
        """
        Synchronizes the internal module map with the shared ContextData container.
        """
        if self.context_data:
            self.context_data.module_map = self.module_map

    def update_lint_reports(self):
        """
        Synchronizes the internal lint reports with the shared ContextData container.
        """
        if self.context_data:
            self.context_data.lint_reports = self.lint_reports
    ...
    def run_linter(self):
        ...
        self.lint_reports = {}
        try:
            # Run Ruff check on the whole project, excluding noise
            result = subprocess.run(
                ["python", "-m", "ruff", "check", str(self.project_root), "--output-format", "json", "--quiet", "--exclude", ".venv,__pycache__,.git,.backups,.tmp"],
    ...
    def scan_project(self, force: bool = False):
        ...
        for f in py_files:
            ...
            if any(part in rel.split(os.sep) for part in ignored):
                continue
        ...
        for py_file in py_files:
            ...
            # Skip noise
            if any(part in rel_path.split(os.sep) for part in ignored):
                continue
        ...
        self.update_module_map()
        self.update_lint_reports()
    ...
    def get_smart_context(self, prompt: str, target_file: str = "None", max_size: int = 100000, prune: bool = True) -> Tuple[str, str]:
        ...
        # === PHASE 1: DIRECT KEYWORD SCORING ===
        ...
        # inject it at the VERY TOP of the context.
        if self.context_data and self.context_data.last_run_error:
